...
func (s *AuthorizationServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	log.Print(requestToken)
	authoResp, err := s.pocketClient.Authorize(r.Context(), requestToken)
	if err != nil {
		log.Println(err)
	...
	err = s.tokenRepository.Save(chatID, authoResp.AccessToken, repository.AccessToken)
	if err != nil {
		log.Println(err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	...
	log.Printf("chat_id: %d\nrequest_token: %s/naccess_token: %s\n", chatID, requestToken, authoResp.AccessToken)
